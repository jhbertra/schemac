...
defaultMain = runSchemac print (mapM_ printError >=> const exitFailure) (const $ pure ()) $ do
  ...
  where
    ...
    message (DuplicateEntityLink eName lName _) = "Duplicate link declaration \"link " <> lName <> "\""
    message (DuplicateEntityLinkTag eName lName tName _) = "Duplicate tag \"#" <> tName <> "\""
    message (DuplicateEntityProp eName pName _) = "Duplicate prop declaration \"prop " <> pName <> "\""
    message (DuplicateEntityPropTag eName pName tName _) = "Duplicate tag \"#" <> tName <> "\""
